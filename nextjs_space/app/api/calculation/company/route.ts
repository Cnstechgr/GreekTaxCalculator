...
// POST - Save or update company calculation
export async function POST(request: NextRequest) {
  try {
    ...
    // Upsert calculation
    const calculation = await prisma.companyCalculation.upsert({
      where: {
        businessId_year: {
          businessId,
          year: parseInt(year),
    ...
    console.error('Company calculation save error:', error);
    return NextResponse.json(
...
// GET - Fetch company calculations
export async function GET(request: NextRequest) {
  try {
    ...
    const calculations = await prisma.companyCalculation.findMany({
      where,
      orderBy: { year: 'desc' },
    });
    ...
    console.error('Company calculation fetch error:', error);
    return NextResponse.json(
